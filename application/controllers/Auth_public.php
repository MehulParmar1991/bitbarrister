...
class Auth_public extends CI_Controller {
    ...
    function dashboard() {        
        if ($this->flexi_auth->get_user_group_id() == 3) {
            echo "Login Successfull For " . $this->flexi_auth->get_user_group_id() . 'Admin Account !!';
            die();
        } elseif ($this->flexi_auth->get_user_group_id() == 1) {
            redirect("member/account");
        } else {
            echo "Redirected because valid moderator login!!!";
        }
    }
}
