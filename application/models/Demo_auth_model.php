...
class Demo_auth_model extends CI_Model {
    ...
    function register_account() {
        ...
        if ($this->form_validation->run()) {
            ...
            $profile_data = array(
                'uadd_address_01' => $this->input->post('user_address'),
                'upro_fathername' => $this->input->post('father_name'),
                'upro_familyname' => $this->input->post('family_name'),
                'whatsapp_no' => $this->input->post('whatsapp_no'),
                'other_mobileno' => $this->input->post('other_mobileno'),
            );
            ...
            if ($response) {
                ...
                $family_info = $this->input->post('person_name');
                $gender_array = $this->input->post('person_gender');
                $birthdate_array = $this->input->post('person_birthdate');
                $marital_array = $this->input->post('marital_status');
                $education_array = $this->input->post('education');

                $family_array = array();
                foreach ($family_info as $key => $family) {
                    $family_info_array = array(
                        'user_id' => $response,
                        'name' => $family_info[$key],
                        'gender' => $gender_array[$key],
                        'date_of_birth' => $birthdate_array[$key],
                        'marital_status' => $marital_array[$key],
                        'education' => $education_array[$key],
                        'status' => 0
                    );
                    if ($family_info[$key] != "") {
                        array_push($family_array, $family_info_array);
                    }
                }
                if (!empty($family_array)) {
                    $this->db->insert_batch('family_info', $family_array);
                }
